...
class Notify extends Controller
{
  ...
  /**
    ...
    public function paysuccess()
    {
        ...
        $orderData = db('goods_order') -> field('status,admin_id')-> where("order_sn",$notify->data["out_trade_no"]) -> find();
        logFile(__FUNCTION__,$orderData);
        $adminData = db('Admin') -> field('mch_secret,sms_switch,sms_phone,sms_account_number,sms_password,sms_signature,email_switch,email_to') -> find($orderData['admin_id']);
        logFile(__FUNCTION__,$adminData);
        $notify -> setParameter("key",$adminData['mch_secret']);
        ...
        if($checkSign == TRUE){
            if ($notify->data["return_code"] == "FAIL") {
                ...
                if($orderData['status']==1){
                    ...
                    $Goodsapi->payGold($trade_no,$transaction_id);
                    $success = 1;
                }else{
                    $success=0;
                }
